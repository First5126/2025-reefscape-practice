...
public class RobotContainer {
  ...
  private final SwerveRequest.FieldCentric drive = new SwerveRequest.FieldCentric().withDeadband(MaxSpeed * 0.1).withRotationalDeadband(MaxAngularRate * 0.1); // Add a 10% deadband.withDriveRequestType(DriveRequestType.OpenLoopVoltage); // Use open-loop control for drive motors
  private final SwerveRequest.RobotCentric driveRobCentric = new SwerveRequest.RobotCentric().withDriveRequestType(DriveRequestType.OpenLoopVoltage); // Use open-loop control for drive motors
  ...
  private void configureBindings() {
    ...
    m_driverController.povUp().and(this::yPressed).onFalse(m_elevator.stopMotors());
    m_driverController.povDown().and(this::yPressed).onFalse(m_elevator.stopMotors());

    // Replace with an existing method or define followWithPathWithAction() in CommandXboxController
    m_driverController.a().whileTrue(m_coralRollers.placeCoral());
